...
interface ReportStats {
  ...
  planDistribution: { freemium: number; pro: number; business: number };
}
  ...
  useEffect(() => {
    const loadReportsData = async () => {
      ...
      try {
        ...
        const totalRevenue = sales.reduce((total: number, sale) => {
          return total + sale.amount;
        }, 0);

        const projectsThisMonth = users.reduce((total: number, u) => total + (u.projects_generated || 0), 0);

        const planDistribution = {
          freemium: users.filter((u) => u.plan_type === 'freemium').length,
        ...
        // Simulate activity recente - This should be replaced with real activity data
        const activity = users.slice(-10).map((user) => ({
          id: user.id,
